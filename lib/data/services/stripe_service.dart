...
class StripeService {
  ...
  Future<void> initialize(
    ...
    try {
      ...
      if (merchantIdentifier != null && merchantIdentifier.isNotEmpty) {
        ...
        print(
          'ℹ️ No merchant identifier provided - Apple Pay will not be available',
        );
        print(
          '  To enable Apple Pay, set merchantIdentifier when initializing Stripe',
        );
      }
      ...
      if (kDebugMode) {
        ...
        if (Platform.isIOS) {
          print('  - Platform: iOS');
          print(
            '  - URL Scheme: com.example.amorra (configured in Info.plist)',
          );
  ...
  Future<Map<String, String>> createPaymentIntent({
    ...
    try {
      ...
      final url = Uri.parse(
        '$baseUrl${ApiConstants.endpointCreatePaymentIntent}',
      );
      ...
      if (baseUrl.isEmpty || !url.hasScheme || !url.hasAuthority) {
        throw Exception(
          'Invalid backend URL: $baseUrl. Please check your API_BASE_URL in .env file.',
        );
      }
      ...
      if (kDebugMode) {
        ...
        print(
          '  - Current URL: ${dotenv.env['API_BASE_URL'] ?? ApiConstants.baseUrl}',
        );
      }
      throw Exception(
  ...
  /// - Decline: 4000 0000 0000 0002
  /// - Any future expiry date, any CVC
  Future<bool> confirmPayment({required String clientSecret}) async {
    try {
      if (kDebugMode) {
  ...
  Future<String> createSubscription({
    ...
    try {
      ...
      throw UnimplementedError(
        'Subscription creation should be done on backend',
      );
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> handleStripeRedirect(String url) async {
    try {
      ...
      if (kDebugMode) {
        ...
        print(
          'TODO: Implement handleURLCallback based on your Stripe SDK version',
        );
      }
